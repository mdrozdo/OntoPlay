...
public class JenaKBGridTest {
    ...
    @Test
    public void getClass_ReturnedPropertiesIncludeDomain() throws Exception {
        OntoClass memoryClass = kb.getOwlClass("http://gridagents.sourceforge.net/AiGGridOntology#PhysicalMemory");

        assertThat(memoryClass).isNotNull();
        List<OwlElement> domainElements = memoryClass.getProperties().stream()
                .filter(p -> "hasTotalSize".equalsIgnoreCase(p.getLocalName()))
                .findAny()
                .map(p->p.getDomain())
                .get();
    ...
    public void withoutIgnoreEmptyDomains_getClass_includesPropertyWithNoDomain() throws Exception {
        ...
        OntologyReader specialReader = kbFactory.create("./AiGGridOntology.owl", "http://gridagents.sourceforge.net/AiGGridOntology", Arrays.asList(new FolderMapping("http://purl.org/NET/cgo", "./cgo.owl")), false);
